...
import 'package:Kootumb/models/post_links_list.dart';
...
class Post extends UpdatableModel<Post> {
  ...
  Map<String, dynamic> toJson() {
    return {
      ...
      'reactions_emoji_counts': reactionsEmojiCounts?.counts
          ?.map(
            (ReactionsEmojiCount? reactionEmojiCount) =>
  ...
  void updateFromJson(Map json) {
    if (json.containsKey('reactions_emoji_counts')) {
      reactionsEmojiCounts = factory.parseReactionsEmojiCounts(
        json['reactions_emoji_counts'],
      );
    }
  ...
  bool hasPublicInteractions() {
    return publicReactions != null &&
        areCommentsEnabled != null &&
        publicReactions! &&
        areCommentsEnabled!;
  }
  ...
  PostLink? getLinkToPreview() {
    if (postLinksList != null &&
        postLinksList!.postLinks != null &&
        postLinksList!.postLinks!.isNotEmpty) {
      PostLink? linkPreview = postLinksList!.postLinks!.firstWhereOrNull(
        (link) => link.hasPreview!,
      );
      return linkPreview;
    }
  ...
  bool hasComments() {
    return commentsList != null && commentsList!.comments!.isNotEmpty;
  }

  bool hasCircles() {
    return circles != null && circles!.circles!.isNotEmpty;
  }
  ...
  bool isOlderThan(Duration duration) {
    return created != null &&
        created!.isBefore(DateTime.now().subtract(duration));
  }
  ...
  void setReaction(PostReaction? newReaction) {
    ...
    if (hasReaction) {
      ...
      if (currentReactionEmojiCount?.count != null &&
    ...
    if (newReaction != null) {
      ...
      if (reactionEmojiCount != null) {
        ...
        // Add new emoji count
        newEmojiCounts?.add(
          ReactionsEmojiCount(emoji: newReaction.emoji, count: 1),
        );
      }
    }

    reaction = newReaction;
    _setReactionsEmojiCounts(
  ...
  void _updateHashtagsMap() {
    ...
    Map<String, Hashtag> updatedMap = {};
    hashtagsList?.hashtags?.forEach(
      (hashtag) => updatedMap[hashtag.name!] = hashtag,
    );
...
class PostFactory extends UpdatableModelFactory<Post> {
  @override
  SimpleCache<int, Post>? cache = SimpleCache(
    storage: UpdatableModelSimpleStorage(size: 100),
  );
  ...
  Post makeFromJson(Map json) {
    return Post(
      ...
      reactionsEmojiCounts: parseReactionsEmojiCounts(
        json['reactions_emoji_counts'],
      ),
    );
  }
...
class OBPostStatus {
  ...
  static const draft = OBPostStatus._internal('D');
  static const processing = OBPostStatus._internal('PG');
  static const published = OBPostStatus._internal('P');

  static const _values = <OBPostStatus>[draft, processing, published];

  static List<OBPostStatus> values() => _values;
