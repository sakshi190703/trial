...
import 'package:Kootumb/widgets/buttons/stream_load_more_button.dart';
import 'package:Kootumb/widgets/post/post.dart';
import 'package:Kootumb/widgets/posts_stream/widgets/dr_hoo.dart';
...
class OBPostsStreamState extends State<OBPostsStream>
  ...
  void initState() {
    ...
    _posts = widget.initialPosts?.toList() ?? [];
    ...
    _animation =
        Tween(begin: 1.0, end: 0.0).animate(_hideOverlayAnimationController);
  ...
  Widget _buildStream() {
    ...
    bool hasPrependedItems = widget.prependedItems?.isNotEmpty ?? false;
  ...
  List<Widget> _getPostsStreamStackChildren(List<Widget> streamItems) {
    ...
    stackChildren.add(InViewNotifierList(
  ...
  void _addPostToTop(Post post) {
    setState(() {
      ...
      if (_status == OBPostsStreamStatus.empty) {
...
typedef OBPostsStreamRefresher<Post> = Future<List<Post>> Function();
typedef OBPostsStreamOnScrollLoader<T> = Future<List<Post>> Function(
    List<Post> posts);
typedef OBPostsStreamSecondaryRefresher = Future Function();
