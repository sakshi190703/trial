...
export 'package:Kootumb/widgets/theming/text.dart';
  ...
  matches.addAll(
    _usernameRegex
        ...
        .map((m) {
          ...
          if (group1 != null && group0 != null) {
            return SmartMatch(
              UsernameElement(group1),
              m.start + group0.indexOf("@"),
              m.end,
            );
          }
          return null;
  ...
  matches.addAll(
    _communityNameRegex
        ...
        .map((m) {
          final group0 = m.group(0);
          if (group0 != null) {
            return SmartMatch(CommunityNameElement(group0), m.start, m.end);
          }
          return null;
        })
        .where((element) => element != null)
  ...
  matches.addAll(
    linkRegex
        ...
        .map((m) {
          final group0 = m.group(0);
          if (group0 != null) {
            return SmartMatch(LinkElement(group0), m.start, m.end);
          }
          return null;
        })
        .where((element) => element != null)
  ...
  matches.addAll(
    _tagRegex
        ...
        .map((m) {
          final group0 = m.group(0);
          if (group0 != null) {
            return SmartMatch(HashtagElement(group0), m.start, m.end);
          }
          return null;
        })
        .where((element) => element != null)
...
class OBSmartText extends StatelessWidget {
  ...
  TextSpan _buildSpan({
    ...
    void onUsernameTapped0(String username) {
    ...
    if (trailingSmartTextElement != null) {
      elements.add(trailingSmartTextElement!);
    }
    ...
    return TextSpan(
      children: elements.map<InlineSpan>((element) {
        ...
        if (element is TextElement) {
          textSpan = TextSpan(text: element.text, style: style);
        } else if (element is SecondaryTextElement) {
          textSpan = TextSpan(text: element.text, style: secondaryTextStyle);
        } else if (element is LinkElement) {
          textSpan = LinkTextSpan(
            text: element.text,
            style: linkStyle,
            onPressed: () => onOpen(element.url),
          );
        } else if (element is HashtagElement) {
          String rawHashtagName = element.text.substring(
            1,
            element.text.length,
          );
          String hashtagName = rawHashtagName.toLowerCase();
          if (hashtagsMap != null && hashtagsMap!.containsKey(hashtagName)) {
            Hashtag hashtag = hashtagsMap![hashtagName]!;
            textSpan = WidgetSpan(
          ...
          textSpan = LinkTextSpan(
            text: element.text,
            style: usernameStyle,
            onPressed: () => onUsernameTapped0(element.username),
          );
        } else if (element is CommunityNameElement) {
          textSpan = LinkTextSpan(
            text: element.text,
            style: communityNameStyle,
            onPressed: () => onCommunityNameTapped0(element.communityName),
          );
        }
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder(
      ...
      builder: (BuildContext context, AsyncSnapshot<OBTheme> snapshot) {
        ...
        Color primaryTextColor = themeValueParserService.parseColor(
        ...
        if (trailingSmartTextElement != null) {
          ...
          Color secondaryTextColor = themeValueParserService.parseColor(
...
typedef OnHashtagTapped = Function({Hashtag hashtag, String rawHashtagName});
