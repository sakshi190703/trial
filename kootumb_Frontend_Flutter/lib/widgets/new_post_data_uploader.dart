...
class OBNewPostDataUploaderState extends State<OBNewPostDataUploader>
  ...
  Widget build(BuildContext context) {
    ...
    if (_needsBootstrap) {
      ...
      _mediaPickerService = kongoProvider.mediaService;
      _bootstrap();
    ...
    if (_data.hasMedia()) {
      rowItems.addAll([_buildMediaPreview(), const SizedBox(width: 15)]);
    }
        ...
        children: <Widget>[
          Padding(
            ...
            child: Row(
              ...
              children: <Widget>[
                Expanded(
                  child: Column(children: <Widget>[Row(children: rowItems)]),
                ),
              ],
            ),
          ),
  ...
  Future _uploadPost() async {
    try {
      ...
      if (_data.remainingMediaToCompress != null &&
          _data.remainingMediaToCompress!.isNotEmpty) {
        _setStatusMessage(
      ...
      if (_data.createdDraftPostStatus == null ||
        ...
        _checkPostStatusTimer = Timer.periodic(const Duration(seconds: 1), (
          timer,
        ) async {
      ...
      if (error is HttpieConnectionRefusedError) {
        ...
        _setStatusMessage(
          errorMessage ?? _localizationService.error__unknown_error,
        );
      } else {
        _setStatusMessage(
  ...
  Future _getPublishedPost() async {
    ...
    Post publishedPost = await _userService.getPostWithUuid(
      _data.createdDraftPost!.uuid!,
    );
  ...
  Future _compressPostMediaItem(File postMediaItem) async {
    ...
    if (mediaMimeType == 'image') {
      File compressedImage = await _mediaPickerService.compressImage(
        postMediaItem,
      ) as File;
      _data.remainingCompressedMediaToUpload.add(compressedImage);
      if (compressedImage.path.contains('compressed')) {
      ...
      if (compressedVideo.path.contains('compressed')) {
  ...
  Future _addPostMedia() {
    ...
    return Future.wait(
      _data.remainingCompressedMediaToUpload.map(_uploadPostMediaItem).toList(),
    );
  }

  Future _uploadPostMediaItem(File file) async {
    await _userService.addMediaToPost(
      file: file,
      post: _data.createdDraftPost!,
    );
  ...
  Future<File> _getMediaThumbnail() async {
    ...
    if (mediaMimeType == 'image') {
      ...
      mediaThumbnail = await _mediaPickerService.getVideoThumbnail(
        mediaToPreview,
      );
    } else {
  ...
  Widget _buildStatusText() {
    return Expanded(
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: <Widget>[OBText(_statusMessage, textAlign: TextAlign.left)],
        ),
      ),
    );
  }
  ...
  Widget _buildActionButtons() {
    ...
    return Row(children: activeActions);
  }
  ...
  Widget _buildRetryButton() {
    return GestureDetector(
      ...
      child: Padding(padding: EdgeInsets.all(10), child: OBIcon(OBIcons.retry)),
    );
  }
  ...
  void _removeMediaFromCache() async {
    ...
    for (var mediaObject in _data.compressedMedia) {
      mediaObject.delete();
    }
