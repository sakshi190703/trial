...
class OBPostReactions extends StatefulWidget {
  ...
  const OBPostReactions(this.post, {Key? key}) : super(key: key);
...
class OBPostReactionsState extends State<OBPostReactions> {
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder(
      ...
      builder: (BuildContext context, AsyncSnapshot<Post> snapshot) {
        ...
        if (emojiCounts?.length == 0) return const SizedBox();
  ...
  void _onEmojiReactionCountPressed(
    ReactionsEmojiCount pressedEmojiCount,
  ) async {
    ...
    if (reacted) {
      ...
      // React
      PostReaction newPostReaction = await _reactToPost(
        pressedEmojiCount.emoji!,
      );
    ...
    try {
      _requestOperation = CancelableOperation.fromFuture(
        _userService.deletePostReaction(
          postReaction: widget.post.reaction!,
          post: widget.post,
        ),
      );
