...
import 'package:Kootumb/models/categories_list.dart';
...
import 'package:Kootumb/models/devices_list.dart';
...
import 'package:Kootumb/models/moderation/moderated_object_list.dart';
import 'package:Kootumb/models/moderation/moderated_object_log_list.dart';
...
import 'package:Kootumb/models/moderation/moderation_penalty_list.dart';
...
import 'package:Kootumb/models/notifications/notifications_list.dart';
...
import 'package:Kootumb/models/post_media_list.dart';
import 'package:Kootumb/models/post_reaction.dart';
import 'package:Kootumb/models/post_reaction_list.dart';
import 'package:Kootumb/models/reactions_emoji_count_list.dart';
...
import 'package:Kootumb/models/trending_posts_list.dart';
import 'package:Kootumb/models/user.dart';
import 'package:Kootumb/models/user_invite.dart';
import 'package:Kootumb/models/user_invites_list.dart';
...
export 'package:Kootumb/services/httpie.dart';
import 'package:path_provider/path_provider.dart';
import 'package:path/path.dart';

// Add the correct import for DiskCache if it exists in your project
...
class UserService {
  ...
  Future<void> deleteAccountWithPassword(String password) async {
    HttpieResponse response = await _authApiService.deleteUser(
      password: password,
    );
  ...
  Future<void> clearCache() async {
    ...
    // If DiskCache is not available, use an alternative cache clearing logic or remove this line.
  ...
  Future<bool> clearTemporaryDirectories() async {
    ...
    try {
      Directory tempDir = Directory(
        (await getApplicationDocumentsDirectory()).path.replaceFirst(
              'Documents',
              'tmp',
            ),
      );
  ...
  void checkAndClearTempDirectories() async {
    ...
    try {
      Directory tempDir = Directory(
        (await getTemporaryDirectory()).path.replaceFirst('Documents', 'tmp'),
      );
      Directory vimediaDir = Directory(
        join(
          (await getApplicationDocumentsDirectory()).path.replaceFirst(
      ...
      Directory videoDirAndroid = Directory(
        join((await getTemporaryDirectory()).path, 'video'),
      );
      Directory mediaCacheDir = Directory(
        join((await getTemporaryDirectory()).path, 'mediaCache'),
      );

      if (tempDir.existsSync()) {
        tempDir.listSync().forEach(
              (var entity) => size += entity.statSync().size,
            );
      }
      if (vimediaDir.existsSync()) {
        vimediaDir.listSync().forEach(
              (var entity) => size += entity.statSync().size,
            );
      }
      if (mediaCacheDir.existsSync()) {
        mediaCacheDir.listSync().forEach(
              (var entity) => size += entity.statSync().size,
            );
      }
      if (videoDirAndroid.existsSync()) {
        videoDirAndroid.listSync().forEach(
              (var entity) => size += entity.statSync().size,
            );
      }
  ...
  Future<int> subscribeToBetaWaitlist({required String email}) async {
    HttpieResponse response = await _waitlistApiService.subscribeToBetaWaitlist(
      email: email,
    );
    _checkResponseIsOk(response);
  ...
  Future<User> refreshUser() async {
    ...
    HttpieResponse response = await _authApiService.getUserWithAuthToken(
      _authToken ?? '',
    );
    _checkResponseIsOk(response);
  ...
  Future<User> updateUserEmail(String email) async {
    HttpieStreamedResponse response = await _authApiService.updateUserEmail(
      email: email,
    );
    _checkResponseIsOk(response);
  ...
  Future<void> loginWithStoredUserData() async {
    ...
    String? userData = await _getStoredUserData();
    if (userData != null) {
  ...
  Future<PostsList> getTimelinePosts({
    ...
    if (cachePosts) {
      _storeFirstPostsData(postsData);
    }
    return _makePostsList(postsData);
  }

  Future<PostsList> getStoredFirstPosts() async {
    String? firstPostsData = await _getStoredFirstPostsData();
  ...
  Future<void> setStoredTopPosts(List<TopPost> topPosts) async {
    String topPostsData = json.encode(
      topPosts.map((TopPost topPost) => topPost.toJson()).toList(),
    );
    await _removeStoredTopPostsData();
    await _storeTopPostsData(topPostsData);
  }

  Future<void> setTopPostsLastViewedId(int lastViewedId) async {
    String topPostId = lastViewedId.toString();
    await _removeStoredTopPostsLastViewedId();
    debugPrint('Setting id $lastViewedId as last viewed post for top posts');
    await _storeTopPostsLastViewedId(topPostId);
  }

  Future<TopPostsList> getStoredTopPosts() async {
    String? topPostsData = await _getStoredTopPostsData();
  ...
  Future<int?> getStoredTopPostsLastViewedId() async {
    String? topPostId = await _getStoredTopPostsLastViewedId();
    if (topPostId != null) {
  ...
  Future<void> addMediaToPost({required File file, required Post post}) async {
    HttpieStreamedResponse response = await _postsApiService.addMediaToPost(
      file: file,
      postUuid: post.uuid!,
    );
  ...
  Future<PostMediaList> getMediaForPost({required Post post}) async {
    HttpieResponse response = await _postsApiService.getPostMedia(
      postUuid: post.uuid!,
    );
  ...
  Future<void> publishPost({required Post post}) async {
    HttpieResponse response = await _postsApiService.publishPost(
      postUuid: post.uuid!,
    );
  ...
  Future<OBPostStatus?> getPostStatus({required Post post}) async {
    HttpieResponse response = await _postsApiService.getPostWithUuidStatus(
      post.uuid!,
    );
  ...
  Future<void> deletePost(Post post) async {
    if (post.uuid == null) {
      throw ArgumentError('Post UUID cannot be null');
    }
    HttpieResponse response = await _postsApiService.deletePostWithUuid(
      post.uuid!,
    );
    _checkResponseIsOk(response);
  }
  ...
  Future<Post> closePost(Post post) async {
    HttpieResponse response = await _postsApiService.closePostWithUuid(
      post.uuid!,
    );
    _checkResponseIsOk(response);
    return Post.fromJson(json.decode(response.body));
  ...
  Future<Post> openPost(Post post) async {
    HttpieResponse response = await _postsApiService.openPostWithUuid(
      post.uuid!,
    );
    _checkResponseIsOk(response);
    return Post.fromJson(json.decode(response.body));
  ...
  Future<void> deletePostReaction({
    required PostReaction postReaction,
    required Post post,
  }) async {
  ...
  Future<PostReactionList> getReactionsForPost(
    Post post, {
    int? count,
    int? maxId,
    Emoji? emoji,
    ...
    HttpieResponse response =
        await _postsApiService.getReactionsForPostWithUuid(
      post.uuid!,
      count: count,
      maxId: maxId,
      emojiId: emoji?.id,
  ...
  Future<Post> mutePost(Post post) async {
    HttpieResponse response = await _postsApiService.mutePostWithUuid(
      post.uuid!,
    );
    _checkResponseIsOk(response);
    return Post.fromJson(json.decode(response.body));
  ...
  Future<Post> unmutePost(Post post) async {
    HttpieResponse response = await _postsApiService.unmutePostWithUuid(
      post.uuid!,
    );
    _checkResponseIsOk(response);
    return Post.fromJson(json.decode(response.body));
  ...
  Future<PostCommentList> getCommentsForPost(
    Post post, {
    int? maxId,
    int? countMax,
    int? minId,
  ...
  Future<PostCommentList> getCommentRepliesForPostComment(
    ...
    HttpieResponse response =
        await _postsApiService.getRepliesForCommentWithIdForPostWithUuid(
      post.uuid!,
      postComment.id!,
      countMax: countMax,
      maxId: maxId,
  ...
  Future<EmojiGroupList> getEmojiGroups() async {
    HttpieResponse response = await _emojisApiService.getEmojiGroups();
  ...
  Future<EmojiGroupList> getReactionEmojiGroups() async {
    HttpieResponse response = await _postsApiService.getReactionEmojiGroups();
  ...
  Future<LanguagesList> getAllLanguages() async {
    HttpieResponse response = await _authApiService.getAllLanguages();
  ...
  Future<void> setNewLanguage(Language newLanguage) async {
    HttpieResponse response = await _authApiService.setNewLanguage(
      newLanguage,
    );
    _checkResponseIsOk(response);
    await refreshUser();
  ...
  Future<User> getUserWithUsername(String username) async {
    HttpieResponse response = await _authApiService.getUserWithUsername(
      username,
      authenticatedRequest: true,
    );
    _checkResponseIsOk(response);
  ...
  Future<UsersList> getUsersWithQuery(String query) async {
    HttpieResponse response = await _authApiService.getUsersWithQuery(
      query,
      authenticatedRequest: true,
    );
    _checkResponseIsOk(response);
  ...
  Future<UsersList> searchLinkedUsers({
    ...
    HttpieResponse response = await _authApiService.searchLinkedUsers(
      query: query,
      count: count,
      withCommunity: withCommunity?.name,
    );
    _checkResponseIsOk(response);
    return UsersList.fromJson(
  ...
  Future<User> blockUser(User user) async {
    if (user.username == null) {
      throw ArgumentError('User username cannot be null');
    }
    HttpieResponse response = await _authApiService.blockUserWithUsername(
      user.username!,
    );
    _checkResponseIsOk(response);
    return User.fromJson(json.decode(response.body));
  ...
  Future<User> unblockUser(User user) async {
    ...
    HttpieResponse response = await _authApiService.unblockUserWithUsername(
      user.username!,
    );
    _checkResponseIsOk(response);
    return User.fromJson(json.decode(response.body));
  ...
  Future<UsersList> searchFollowers({required String query, int? count}) async {
    HttpieResponse response = await _authApiService.searchFollowers(
      query: query,
      count: count,
    );
    _checkResponseIsOk(response);
  ...
  Future<UsersList> getFollowers({
    ...
    HttpieResponse response = await _authApiService.getFollowers(
      count: count,
      maxId: maxId,
    );
    _checkResponseIsOk(response);
  ...
  Future<UsersList> searchFollowings({
    ...
    HttpieResponse response = await _authApiService.searchFollowings(
      query: query,
      count: count,
    );
    _checkResponseIsOk(response);
  ...
  Future<UsersList> getFollowings({
    ...
    HttpieResponse response = await _authApiService.getFollowings(
      count: count,
      maxId: maxId,
    );
    _checkResponseIsOk(response);
  ...
  Future<Follow> followUserWithUsername(
    String username, {
    List<FollowsList> followsLists = const [],
  }) async {
  ...
  Future<User> unFollowUserWithUsername(String username) async {
    HttpieResponse response = await _followsApiService.unFollowUserWithUsername(
      username,
    );
    _checkResponseIsOk(response);
    return User.fromJson(json.decode(response.body));
  ...
  Future<Follow> updateFollowWithUsername(
    String username, {
    List<FollowsList> followsLists = const [],
  }) async {
  ...
  Future<Connection> connectWithUserWithUsername(
    String username, {
    List<Circle> circles = const [],
  }) async {
    HttpieResponse response =
        await _connectionsApiService.connectWithUserWithUsername(
      username,
  ...
  Future<Connection> confirmConnectionWithUserWithUsername(
    String username, {
    List<Circle> circles = const [],
  }) async {
    HttpieResponse response =
        await _connectionsApiService.confirmConnectionWithUserWithUsername(
      username,
      circlesIds: circles.map((circle) => circle.id!).toList(),
    );
    _checkResponseIsOk(response);
  ...
  Future<Connection> updateConnectionWithUsername(
    String username, {
    List<Circle> circles = const [],
  }) async {
    HttpieResponse response =
        await _connectionsApiService.updateConnectionWithUsername(
      username,
      circlesIds: circles.map((circle) => circle.id!).toList(),
    );
    _checkResponseIsOk(response);
  ...
  Future<Circle> updateConnectionsCircle(
    Circle circle, {
    String? name,
    String? color,
    ...
    HttpieResponse response =
        await _connectionsCirclesApiService.updateCircleWithId(
      circle.id!,
      name: name,
      color: color,
  ...
  Future<FollowsList> updateFollowsList(
    FollowsList list, {
    String? name,
    Emoji? emoji,
    List<User>? users,
  ...
  Future<void> deleteFollowsList(FollowsList list) async {
    HttpieResponse response = await _followsListsApiService.deleteListWithId(
      list.id!,
    );
    _checkResponseIsOk(response);
  }
  ...
  Future<FollowsList> getFollowsListWithId(int listId) async {
    HttpieResponse response = await _followsListsApiService.getListWithId(
      listId,
    );
    _checkResponseIsOk(response);
    return FollowsList.fromJSON(json.decode(response.body));
  ...
  Future<void> deleteUserInvite(UserInvite userInvite) async {
    HttpieResponse response = await _userInvitesApiService.deleteUserInvite(
      userInvite.id!,
    );
    _checkResponseIsOk(response);
  }
  ...
  Future<Post> createPostForCommunity(
    Community community, {
    String? text,
    File? image,
    File? video,
    ...
    HttpieStreamedResponse response =
        await _communitiesApiService.createPostForCommunityWithId(
      community.name!,
      text: text,
      image: image,
      video: video,
  ...
  Future<PostsList> getPostsForCommunity(
    Community community, {
    int? maxId,
    int? count,
  }) async {
    HttpieResponse response =
        await _communitiesApiService.getPostsForCommunityWithName(
      community.name!,
      count: count,
      maxId: maxId,
    );
  ...
  Future<int?> countPostsForCommunity(
    Community community, {
    int? maxId,
    int? count,
  }) async {
    ...
    _checkResponseIsOk(response);
    Community responseCommunity = Community.fromJSON(
      json.decode(response.body),
    );
    return responseCommunity.postsCount;
  }

  Future<PostsList> getClosedPostsForCommunity(
    Community community, {
    int? maxId,
    int? count,
  }) async {
    HttpieResponse response =
        await _communitiesApiService.getClosedPostsForCommunityWithName(
      community.name!,
      count: count,
      maxId: maxId,
    );
  ...
  Future<CommunitiesList> searchCommunitiesWithQuery(
    String query, {
    ...
    HttpieResponse response =
      ...
      query: query,
      excludedFromProfilePosts: excludedFromProfilePosts,
    );
    _checkResponseIsOk(response);
    return CommunitiesList.fromJson(json.decode(response.body));
  }
  ...
  Future<Community> updateAvatarForCommunity(
    Community community, {
  ...
  Future<Community> updateCoverForCommunity(
    Community community, {
    required File cover,
  }) async {
    HttpieStreamedResponse response = await _communitiesApiService
  ...
  Future<Community> getCommunityWithName(String name) async {
    HttpieResponse response = await _communitiesApiService.getCommunityWithName(
      name,
    );
    _checkResponseIsOk(response);
  ...
  Future<UsersList> getMembersForCommunity(
    Community community, {
    int? count,
    int? maxId,
    List<CommunityMembersExclusion>? exclude,
  }) async {
    HttpieResponse response =
        await _communitiesApiService.getMembersForCommunityWithId(
      community.name!,
      count: count,
      maxId: maxId,
      exclude: exclude
          ?.map(
            (exclude) => Community.convertExclusionToString(exclude),
          )
          .toList(),
    );
  ...
  Future<Community> joinCommunity(Community community) async {
    HttpieResponse response = await _communitiesApiService.joinCommunityWithId(
      community.name!,
    );
    _checkResponseIsCreated(response);
    return Community.fromJSON(json.decode(response.body));
  ...
  Future<Community> leaveCommunity(Community community) async {
    HttpieResponse response = await _communitiesApiService.leaveCommunityWithId(
      community.name!,
    );
    _checkResponseIsOk(response);
    return Community.fromJSON(json.decode(response.body));
  ...
  Future<UsersList> getModeratorsForCommunity(
    Community community, {
    int? count,
    int? maxId,
  }) async {
    HttpieResponse response =
        await _communitiesApiService.getModeratorsForCommunityWithId(
      community.name!,
      count: count,
      maxId: maxId,
    );
  ...
  Future<UsersList> getAdministratorsForCommunity(
    Community community, {
    int? count,
    int? maxId,
  }) async {
    HttpieResponse response =
        await _communitiesApiService.getAdministratorsForCommunityWithName(
      community.name!,
      count: count,
      maxId: maxId,
    );
  ...
  Future<UsersList> getBannedUsersForCommunity(
    Community community, {
    int? count,
    int? maxId,
  }) async {
    HttpieResponse response =
        await _communitiesApiService.getBannedUsersForCommunityWithId(
      community.name!,
      count: count,
      maxId: maxId,
    );
  ...
  Future<HashtagsList> getHashtagsWithQuery(String query) async {
    HttpieResponse response = await _hashtagsApiService.getHashtagsWithQuery(
      query: query,
    );
    _checkResponseIsOk(response);
  ...
  Future<PostsList> getPostsForHashtag(
    Hashtag hashtag, {
    int? maxId,
    int? count,
  }) async {
  ...
  Future<Hashtag> getHashtagWithName(String name) async {
    HttpieResponse response = await _hashtagsApiService.getHashtagWithName(
      name,
    );
    _checkResponseIsOk(response);
  ...
  Future<void> deleteDevice(Device device) async {
    HttpieResponse response = await _devicesApiService.deleteDeviceWithUuid(
      device.uuid!,
    );
    _checkResponseIsOk(response);
  }
  ...
  Future<Device> getDeviceWithUuid(String deviceUuid) async {
    HttpieResponse response = await _devicesApiService.getDeviceWithUuid(
      deviceUuid,
    );
    _checkResponseIsOk(response);
    return Device.fromJSON(json.decode(response.body));
  ...
  Future<Device> getOrCreateCurrentDevice() async {
    if (_getOrCreateCurrentDeviceCache != null) {
      return await _getOrCreateCurrentDeviceCache!;
    }
    ...
    return await (_getOrCreateCurrentDeviceCache ??
        Future.error('Device cache is null'));
  }
  ...
  Future<void> reportUser({
    required User user,
    String? description,
    required ModerationCategory moderationCategory,
  }) async {
  ...
  Future<ModeratedObjectsList> getGlobalModeratedObjects({
    ...
    HttpieResponse response =
        await _moderationApiService.getGlobalModeratedObjects(
      maxId: maxId,
      verified: verified,
      types: types
          ?.map(
  ...
  Future<ModeratedObjectsList> getCommunityModeratedObjects({
    ...
    HttpieResponse response = await _communitiesApiService.getModeratedObjects(
      ...
      statuses: statuses
          ?.map(
            (status) => ModeratedObject.factory.convertStatusToString(status)!,
          )
          .toList(),
      count: count,
    );
  ...
  Future<void> updateModeratedObject(
    ModeratedObject moderatedObject, {
    String? description,
    ModerationCategory? category,
  }) async {
    HttpieResponse response =
        await _moderationApiService.updateModeratedObjectWithId(
      moderatedObject.id!,
      description: description,
  ...
  Future<ModeratedObjectLogsList> getModeratedObjectLogs(
    ...
    HttpieResponse response =
        await _moderationApiService.getModeratedObjectLogs(
      moderatedObject.id!,
      maxId: maxId,
      count: count,
    );
  ...
  Future<ModerationReportsList> getModeratedObjectReports(
    ...
    HttpieResponse response =
        await _moderationApiService.getModeratedObjectReports(
      moderatedObject.id!,
      maxId: maxId,
      count: count,
    );
  ...
  Future<String> translatePost({required Post post}) async {
    HttpieResponse response = await _postsApiService.translatePost(
      postUuid: post.uuid!,
    );
  ...
  Future<bool> linkIsPreviewable({required String link}) async {
    HttpieResponse response = await _postsApiService.linkIsPreviewable(
      link: link,
    );
    _checkResponseIsOk(response);
  ...
  Map<UserVisibility, Map<String, String>> getUserVisibilityLocalizationMap() {
    ...
    var kootumbMap = {
      'title': _localizationService.user__visibility_kootumb,
      'description': _localizationService.user__visibility_kootumb_desc,
    };
    ...
    return {
      ...
      UserVisibility.kootumb: kootumbMap,
  ...
  Future<void> setLanguageFromDefaults() async {
    ...
    // Language? deviceLanguage = languageList.languages?.firstWhere(
    //   (
    //     Language language,
    //   ) {
    ...
    if (languageList.languages != null) {
      deviceLanguage = languageList.languages!.firstWhere(
        (language) =>
            language.code?.toLowerCase() ==
            currentLocale.languageCode.toLowerCase(),
        orElse: () => Language(), // 🟡 Temporary fallback — see note below
      );

      // If fallback is not desirable, add a manual null check:
      if (deviceLanguage.code == null) {
        deviceLanguage = null; // treat the fallback as "no match"
      }
    }

    if (deviceLanguage != null && deviceLanguage.id != null) {
      ...
      try {
        if (languageList.languages != null) {
          ...
          if (english.id != null) {
            ...
            print(
                'English language found but has no ID, skipping language setup');
          }
        }
      } catch (e) {
        print(
            'No English language found in API response, skipping language setup');
      }
    }
  }
