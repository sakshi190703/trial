...
class StorageService {
  ...
  OBStorage getSystemPreferencesStorage({required String namespace}) {
    return OBStorage(
      store: _SystemPreferencesStorage(namespace),
      namespace: namespace,
    );
  }
}
...
class OBStorage {
  ...
  Future<String?> get(String key, {String? defaultValue}) async {
    ...
    String? value = await store.get(finalKey);
  ...
  Future<void> set(String key, dynamic value) {
    return store.set(_makeKey(key), value);
  }

  Future<void> remove(String key) {
    return store.remove(_makeKey(key));
  }
...
class _SecureStore implements _Store<String> {
  ...
  void _loadPreviouslyStoredKeys() async {
    ...
    List<String>? storedKeys = preferences.getStringList(
      'secure_store.keylist',
    );
