...
class MediaService {
  ...
  Future<File?> pickImage({
    ...
    File? pickedImage = await _bottomSheetService.showImagePicker(
      context: context,
    );
  ...
  Future<File?> pickVideo({required BuildContext context}) async {
    File? pickedVideo = await _bottomSheetService.showVideoPicker(
      context: context,
    );
  ...
  Future<MediaFile?> processMedia({
    ...
    if (await isGif(media.file) && !flattenGifs) {
      ...
      convertGifToVideo(copiedFile).then(
        (file) => completer.complete(file),
        onError: (error, trace) {
          print(error);
          _toastService.error(
  ...
  Future<File> copyMediaFile(File mediaFile, {deleteOriginal = true}) async {
    ...
    File fileCopy = mediaFile.copySync(
      '$tempPath/$processedImageUuid$imageExtension',
    );
  ...
  Future<String> _getTempPath() async {
    ...
    Directory mediaCacheDir = Directory(
      join(applicationsDocumentsDir.path, 'mediaCache'),
    );
  ...
  // Removed duplicate and obsolete compressVideo method using FlutterFFmpeg.
  ...
  CancelableOperation<File> convertGifToVideo(File gif) {
    ...
    var ffmpegFuture = _convertGifToVideo(
      gif,
      () => isCancelled,
    );
    return CancelableOperation.fromFuture(
      ...
      onCancel: () {
        isCancelled = true;
        // _flutterFFmpeg.cancel(); // ffmpeg_kit_flutter does not support cancel directly
      },
    );
  }
  ...
  Future<Future<CroppedFile?>> cropImage(
    File image, {
    double? ratioX,
    double? ratioY,
  }) async =>
