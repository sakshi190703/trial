...
class UtilsService {
  ...
  static SimpleCache<String, bool> hexColorIsDarkCache = SimpleCache(
    storage: InMemoryStorage(30),
  );

  static SimpleCache<String, Color> parseHexColorCache = SimpleCache(
    storage: InMemoryStorage(30),
  );

  static RegExp hashtagsRegExp = RegExp(
    r"\B#\w*[a-zA-Z]+\w*",
    caseSensitive: false,
  );
  ...
  String timeAgo(DateTime date, LocalizationService localizationService) {
    /// Originally from https://gist.github.com/DineshKachhot/bc8cee616f30c323c1dd1e63a4bf65df
    final now = DateTime.now();
    final difference = now.difference(date);
    String years = localizationService.post__time_short_years;
    String weeks = localizationService.post__time_short_weeks;
    String days = localizationService.post__time_short_days;
    String hours = localizationService.post__time_short_hours;
    String mins = localizationService.post__time_short_minutes;
    String seconds = localizationService.post__time_short_seconds;

    if ((difference.inDays / 365).floor() >= 2) {
      return '${(difference.inDays / 365).floor()}$years';
    } else if ((difference.inDays / 365).floor() >= 1) {
      return localizationService.post__time_short_one_year;
    } else if ((difference.inDays / 7).floor() >= 2) {
      return '${(difference.inDays / 7).floor()}$weeks';
    } else if ((difference.inDays / 7).floor() >= 1) {
      return localizationService.post__time_short_one_week;
    } else if (difference.inDays >= 2) {
      return '${difference.inDays}$days';
    } else if (difference.inDays >= 1) {
      return localizationService.post__time_short_one_day;
    } else if (difference.inHours >= 2) {
      return '${difference.inHours}$hours';
    } else if (difference.inHours >= 1) {
      return localizationService.post__time_short_one_hour;
    } else if (difference.inMinutes >= 2) {
      return '${difference.inMinutes}$mins';
    } else if (difference.inMinutes >= 1) {
      return localizationService.post__time_short_one_minute;
    } else if (difference.inSeconds >= 3) {
      return '${difference.inSeconds}$seconds';
    } else {
      return localizationService.post__time_short_now_text;
    }
  }
  ...
  String? getLinkToPreviewFromText(String text) {
    ...
    String? previewUrl;
    matches.addAll(
      linkRegex.allMatches(text).map((match) {
