...
import 'package:Kootumb/plugins/share/share.dart';
...
class ShareService {
  static const _stream = EventChannel('kootumb.com/receive_share');
  ...
  ShareService() {
    ...
    if (Platform.isAndroid) {
      try {
        _shareReceiveSubscription ??= _stream.receiveBroadcastStream().listen(
              _onReceiveShare,
            );
      } catch (e) {
        print('Share plugin not available: $e');
        // Continue without share functionality
      }
    }
  }
  ...
  void unsubscribe(
    Future<dynamic> Function({String text, File image, File video}) subscriber,
  ) {
  ...
  Future<void> _processQueuedShare() async {
    if (_queuedShare != null) {
      ...
      _activeShares.forEach(
        (key, value) => Future.delayed(Duration(), value.cancel),
      );
