...
import 'package:Kootumb/pages/home/pages/communities/communities.dart';
import 'package:Kootumb/pages/home/pages/notifications/notifications.dart';
import 'package:Kootumb/pages/home/pages/own_profile.dart';
import 'package:Kootumb/pages/home/pages/timeline/timeline.dart';
import 'package:Kootumb/pages/home/pages/menu/menu.dart';
import 'package:Kootumb/pages/home/pages/search/search.dart';
import 'package:Kootumb/pages/home/widgets/bottom-tab-bar.dart';
import 'package:Kootumb/pages/home/widgets/own_profile_active_icon.dart';
import 'package:Kootumb/pages/home/widgets/tab-scaffold.dart';
...
class OBHomePage extends StatefulWidget {
  const OBHomePage({Key? key}) : super(key: key);
...
class OBHomePageState extends State<OBHomePage> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    // Add this to navigate to Child Safety page after Home loads
    // WidgetsBinding.instance.addPostFrameCallback((_) {
    //   if (!_childSafetyAccepted) {
    //     Navigator.pushNamed(context, '/child_safety').then((_) {
    //       _childSafetyAccepted = true;
    //     });
    //   }
    // });
  }
  ...
  Widget build(BuildContext context) {
    if (_needsBootstrap) {
      ...
      _pushNotificationsService = kongoProvider.pushNotificationsService;
      _intercomService = kongoProvider.intercomService;
  ...
  Widget _getPageForTabIndex(int index) {
    ...
    switch (OBHomePageTabs.values[index]) {
      case OBHomePageTabs.timeline:
        page = OBTimelinePage(controller: _timelinePageController);
        break;
      case OBHomePageTabs.search:
        page = OBMainSearchPage(controller: _searchPageController);
        break;
      case OBHomePageTabs.notifications:
        page = OBNotificationsPage(controller: _notificationsPageController);
        break;
      case OBHomePageTabs.communities:
        page = OBMainCommunitiesPage(controller: _communitiesPageController);
        break;
      case OBHomePageTabs.profile:
        page = OBOwnProfilePage(controller: _ownProfilePageController);
        break;
      case OBHomePageTabs.menu:
        page = OBMainMenuPage(controller: _mainMenuPageController);
        break;
      default:
        ...
        BottomNavigationBarItem(
          ...
          icon: Stack(
            ...
            children: <Widget>[
              ...
              _loggedInUserUnreadNotifications > 0
                  ? Positioned(right: -8, child: OBBadge(size: 10))
                  : const SizedBox(),
            ],
          ),
          activeIcon: const OBIcon(
  ...
  void _bootstrap() async {
    _loggedInUserChangeSubscription = _userService.loggedInUserChange.listen(
  ...
  Future _logout({unsubscribePushNotifications = false}) async {
    try {
      if (unsubscribePushNotifications) {
        await _pushNotificationsService.disable();
      }
    } catch (error) {
      rethrow;
    } finally {
  ...
  void _onLoggedInUserChange(User? newUser) async {
    if (newUser == null) {
      ...
      _loggedInUserUpdateSubscription = newUser.updateSubject.listen(
        _onLoggedInUserUpdate,
      );
      ...
      //     newUser.areGuidelinesAccepted == null &&
