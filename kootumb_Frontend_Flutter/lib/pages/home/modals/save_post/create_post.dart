...
import 'package:Kootumb/pages/home/modals/save_post/widgets/create_post_text.dart';
import 'package:Kootumb/pages/home/modals/save_post/widgets/post_community_previewer.dart';
import 'package:Kootumb/pages/home/modals/save_post/widgets/post_image_previewer.dart';
import 'package:Kootumb/pages/home/modals/save_post/widgets/post_video_previewer.dart';
import 'package:Kootumb/pages/home/modals/save_post/widgets/remaining_post_characters.dart';
...
import 'package:Kootumb/widgets/avatars/logged_in_user_avatar.dart';
import 'package:Kootumb/widgets/buttons/button.dart';
import 'package:Kootumb/widgets/buttons/pill_button.dart';
...
class OBSavePostModalState extends OBContextualSearchBoxState<OBSavePostModal> {
  ...
  void bootstrap() {
    ...
    if (!_isEditingPost && widget.community != null) {
      _postItemsWidgets.add(
        OBPostCommunityPreviewer(community: widget.community!),
      );
    }

    _textController.addListener(_onPostTextChanged);
    _charactersCount = _textController.text.length;
    _isPostTextAllowedLength = _validationService.isPostTextAllowedLength(
  ...
  Widget build(BuildContext context) {
    if (_needsBootstrap) {
      ...
      _mediaService = kongoProvider.mediaService;
      ...
      _draftService = kongoProvider.draftService;
      _shareService = kongoProvider.shareService;
  ...
  ObstructingPreferredSizeWidget _buildNavigationBar(
    ...
    return OBThemedNavigationBar(
      leading: GestureDetector(
        child: OBIcon(
          OBIcons.close,
          semanticLabel: localizationService.post__close_create_post_label,
        ),
        onTap: () {
          if (_postImageFile != null) _postImageFile!.delete();
          if (_postVideoFile != null) {
            _mediaService.clearThumbnailForFile(_postVideoFile!);
          }
          if (_postVideoFile != null) _postVideoFile!.delete();
          Navigator.pop(context);
        },
      ),
      title: _isEditingPost
          ? localizationService.post__edit_title
          : localizationService.trans('post__create_new'),
  ...
  void _onWantsToGoNext() async {
    ...
    OBNewPostData? createPostData =
        await _navigationService.navigateToSharePost(
      context: context,
      createPostData: _makeNewPostData(),
    );
  ...
  Widget _buildNewPostContent() {
    return Row(
      ...
      children: <Widget>[
        Column(
          children: <Widget>[
            OBLoggedInUserAvatar(size: OBAvatarSize.medium),
  ...
  Widget _buildPostActions() {
    ...
    return ListView.separated(
      ...
      itemBuilder: (BuildContext context, index) {
        ...
        return index == 0
            ? Row(children: <Widget>[const SizedBox(width: 10), postAction])
            : postAction;
      },
  ...
  void _setPostVideo(PostVideo postVideo) {
    ...
    var postVideoWidget = OBPostVideoPreview(postVideo: _postVideo);
  ...
  void _setLinkPreview(LinkPreview linkPreview) {
    ...
    setState(() {
      ...
      _linkPreviewWidgetRemover = _addPostItemWidget(
        OBLinkPreview(linkPreview: _linkPreview),
      );
    });
  }
  ...
  void _onError(error) async {
    if (error is HttpieConnectionRefusedError) {
      ...
      _toastService.error(
        message:
            errorMessage ?? _localizationService.trans('error__unknown_error'),
        context: context,
      );
  ...
  void _removePostImageFile() {
    setState(() {
      if (_postImageFile != null) _postImageFile!.deleteSync();
      _postImage = null;
      _hasImage = false;
  ...
  void _removePostVideoFile() {
    ...
    setState(() {
      _mediaService.clearThumbnailForFile(_postVideoFile!);
      if (_postVideoFile != null) _postVideoFile!.deleteSync();
      _postVideoFile = null;
      _hasVideo = false;
