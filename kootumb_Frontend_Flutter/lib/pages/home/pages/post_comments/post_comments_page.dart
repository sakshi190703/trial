...
import 'package:Kootumb/pages/home/lib/draft_editing_controller.dart';
import 'package:Kootumb/pages/home/pages/post_comments/post_comments_page_controller.dart';
import 'package:Kootumb/pages/home/pages/post_comments/widgets/post_commenter.dart';
import 'package:Kootumb/pages/home/pages/post_comments/widgets/post_comment/post_comment.dart';
import 'package:Kootumb/pages/home/pages/post_comments/widgets/post_comments_header_bar.dart';
import 'package:Kootumb/services/draft.dart';
import 'package:Kootumb/pages/home/pages/post_comments/widgets/post_preview.dart';
...
import 'package:Kootumb/widgets/contextual_search_boxes/contextual_search_box_state.dart';
...
import 'package:Kootumb/widgets/load_more.dart';
...
class OBPostCommentsPageState
  ...
  void initState() {
    ...
    if (widget.linkedPostComment != null) {
      _post = widget.linkedPostComment!.post!;
    }
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.pageType == PostCommentsPageType.comments) {
      _pageTextMap = getPageCommentsMap(_localizationService);
    } else {
      _pageTextMap = getPageRepliesMap(_localizationService);
    }
  ...
  List<Widget> _getStackChildren() {
    ...
    if (_shouldHideStackedLoadingScreen) {
      stackChildren.add(Column(children: _buildPostPageContentItems()));
    } else {
      stackChildren.addAll([
        Column(children: _buildPostPageContentItems()),
        Positioned(
          top: 0.0,
          left: 0.0,
          right: 0.0,
  ...
  List<Widget> _buildPostPageContentItems() {
    ...
    contentItems.addAll([_buildPostComments(), _buildPostCommenter()]);
                      ...
                      OBPostCommentsHeaderBar(
                        ...
                        onWantsToToggleSortComments: () =>
                            _commentsPageController
                                .onWantsToToggleSortComments(),
                        loadMoreTopComments: () =>
                            _commentsPageController.loadMoreTopComments(),
    ...
    if (isAutocompleting) {
      postCommentsStackItems.add(
        Positioned(
          ...
          bottom: 0,
          top: 0,
          child: OBPrimaryColorContainer(child: buildSearchBox()),
        ),
      );
    }

    return Expanded(child: Stack(children: postCommentsStackItems));
  }
  ...
  void _onPostCommentCreated(PostComment createdPostComment) {
    _commentsPageController.refreshCommentsWithCreatedPostCommentVisible(
      createdPostComment,
    );
  ...
  Widget _getCommentTile(int index) {
    ...
    onPostCommentDeletedCallback(PostComment comment) {
      _removePostCommentAtIndex(commentIndex);
      if (widget.onCommentDeleted != null) {
        widget.onCommentDeleted!(postComment);
      }
    }
    ...
    if (widget.linkedPostComment != null &&
      ...
      var theme = _themeService.getActiveTheme();
      var primaryColor = _themeValueParserService.parseColor(
        theme.primaryColor,
      );
  ...
  Widget _getPostPreview() {
    ...
    if ((widget.pageType == PostCommentsPageType.replies &&
      ...
      showViewAllCommentsAction = false;
    }
    ...
    return OBPostPreview(
      ...
      showViewAllCommentsAction: showViewAllCommentsAction,
    );
  }
  ...
  Future<void> _refreshPostComment() async {
    ...
    try {
      ...
      _refreshPostCommentOperation = CancelableOperation.fromFuture(
        _userService.getPostComment(
          post: widget.post!,
          postComment: widget.postComment!,
        ),
      );
  ...
  void _addPostComments(List<PostComment> postComments) {
    ...
    _commentsPageController.updateControllerPostComments(_postComments);
  }

  void _addToStartPostComments(List<PostComment> postComments) {
    for (var comment in postComments.reversed) {
      setState(() {
        _postComments.insert(0, comment);
      });
    }
    _commentsPageController.updateControllerPostComments(_postComments);
  }
  ...
  void _showNoMoreTopItemsToLoadToast() {
    _toastService.info(
      context: context,
      message: _pageTextMap['NO_MORE_TO_LOAD']!,
    );
  }
  ...
  Map<String, String> getPageCommentsMap(
    ...
    return {
      'TITLE': localizationService.post__comments_page_title,
      'NO_MORE_TO_LOAD':
          localizationService.post__comments_page_no_more_to_load,
      'TAP_TO_RETRY': localizationService.post__comments_page_tap_to_retry,
    };
  }
  ...
  Map<String, String> getPageRepliesMap(
    ...
    return {
      'TITLE': localizationService.post__comments_page_replies_title,
      'NO_MORE_TO_LOAD':
          localizationService.post__comments_page_no_more_replies_to_load,
      'TAP_TO_RETRY':
          localizationService.post__comments_page_tap_to_retry_replies,
    };
  }
  ...
  double _calculatePositionTopCommentSection() {
    ...
    if (widget.showPostPreview == true && widget.post != null) {
      ...
      if (_post.hasText()) {
        ...
        TextSpan text = TextSpan(text: postText, style: style);
        ...
        textPainter.layout(
          maxWidth: screenWidth - 40.0,
        ); //padding is 20 in OBPostBodyText
    ...
    if (widget.postComment != null) {
      ...
      TextSpan text = TextSpan(text: commentText, style: style);
      ...
      ); //padding is 100 around comments
