...
# from kongo_auth.models import UserProfile



from kongo.settings import USERNAME_MAX_LENGTH
from kongo_auth.helpers import upload_to_user_cover_directory, upload_to_user_avatar_directory
from kongo_common.peekalink_client import peekalink_client
from kongo_hashtags.queries import make_search_hashtag_query_for_user_with_id, \
    make_get_hashtag_with_name_for_user_with_id_query
from kongo_notifications.helpers import get_notification_language_code_for_target_user
from kongo_posts.queries import make_get_hashtag_posts_for_user_with_id_query
from kongo_posts.query_collections import get_posts_for_user_collection
from kongo_translation import translation_strategy
from kongo_common.helpers import get_supported_translation_language
from kongo_common.models import Badge, Language
from kongo_common.utils.helpers import delete_file_field
from kongo_common.utils.model_loaders import get_connection_model, get_circle_model, get_follow_model, \
...
from kongo_common.validators import name_characters_validator
from kongo_notifications import helpers
from kongo_auth.checkers import *
...
class User(AbstractUser):
    ...
    """
    moderated_object = GenericRelation('kongo_moderation.ModeratedObject', related_query_name='users')
    first_name = None
    last_name = None
    language = models.ForeignKey('kongo_common.Language', null=True, blank=True,
                                 on_delete=models.SET_NULL, related_name='users')
    translation_language = models.ForeignKey('kongo_common.Language', null=True, blank=True,
                                             on_delete=models.SET_NULL, related_name='translation_users')
    email = models.EmailField(_('email address'), unique=True, null=False, blank=False)
    connections_circle = models.ForeignKey('kongo_circles.Circle', on_delete=models.CASCADE, related_name='+',
    ...
    visibility = models.CharField(
        ...
        default=VISIBILITY_TYPE_kootumb,
        max_length=2
    )
    ...
    def update(self,
        ...
        if visibility:
            if self.visibility == User.VISIBILITY_TYPE_PRIVATE and visibility != User.VISIBILITY_TYPE_PRIVATE:
                # We are changing from private to public/kootumb visibility which does not use follow requests
                self.delete_received_follow_requests()

            if self.visibility != User.VISIBILITY_TYPE_PRIVATE and visibility == User.VISIBILITY_TYPE_PRIVATE:
                # We are changing from public/kootumb to private, we need to remove all connection requests.
    ...
    def has_visibility_kootumb(self):
        return self.visibility == User.VISIBILITY_TYPE_kootumb
    ...
    def _send_password_reset_email_with_token(self, password_reset_token):
        mail_subject = _('Reset your password for kootumb')
        text_content = render_to_string('kongo_auth/email/reset_password.txt', {
            'name': self.profile.name,
            'username': self.username,
            'password_reset_link': self._generate_password_reset_link(password_reset_token)
        })

        html_content = render_to_string('kongo_auth/email/reset_password.html', {
            'name': self.profile.name,
            'username': self.username,
            'password_reset_link': self._generate_password_reset_link(password_reset_token)
        })
        ...
        email = EmailMultiAlternatives(
            mail_subject, text_content, to=[self.email], from_email=settings.EMAIL_HOST_USER)
...
class UserNotificationsSubscription(models.Model):
    ...
    def are_new_post_notifications_enabled_for_user_with_username_and_target_with_username(cls, username,
        ...
        return cls.objects.filter(user__username=target_username,
                                  ...
                                  new_post_notifications=True).exists()
@receiver(post_save, sender=settings.AUTH_USER_MODEL, dispatch_uid='bootstrap_user_profile')
def create_user_profile_signal(sender, instance=None, created=False, **kwargs):
    """
    Ensure a UserProfile exists for every user.
    """
    if created and not hasattr(instance, 'profile'):
        # Provide default values for required fields
        UserProfile = sender._meta.apps.get_model('kongo_auth', 'UserProfile')
        UserProfile.objects.create(
            user=instance,
            name=instance.username,  # or any default
            is_of_legal_age=False     # or False, as per your logic
        )