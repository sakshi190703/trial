...
from kongo_common.utils.model_loaders import get_post_model, get_community_model, get_post_comment_model, \
...
from kongo_common import checkers as common_checkers
...
def check_user_visibility_is_not_kootumb(target_user):
    if target_user.has_visibility_kootumb():
        raise ValidationError(
            _('This user content is restricted to kootumb members only.'),
        )
...
def check_can_get_unauthenticated_posts_for_user(user):
    ...
    check_user_visibility_is_not_kootumb(target_user=user)
