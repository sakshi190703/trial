...
from kongo_auth.views.auth.serializers import AuthenticatedUserNotificationsSettingsSerializer, \
    UpdateAuthenticatedUserNotificationsSettingsSerializer
from kongo_auth.views.authenticated_user.serializers import GetAuthenticatedUserSerializer, \
...
from kongo_moderation.permissions import IsNotSuspended, check_user_is_not_suspended
...
class AuthenticatedUserSettings(APIView):
    ...
    def send_confirmation_email(self, user, new_email, confirm_email_token):
        mail_subject = _('Confirm your email for kongo')
        text_content = render_to_string('kongo_auth/email/change_email.txt', {
            'name': user.profile.name,
            'confirmation_link': self.generate_confirmation_link(confirm_email_token)
        })

        html_content = render_to_string('kongo_auth/email/change_email.html', {
            'name': user.profile.name,
            'confirmation_link': self.generate_confirmation_link(confirm_email_token)
        })

        email = EmailMultiAlternatives(
            mail_subject, text_content, to=[new_email], from_email=settings.EMAIL_HOST_USER)
