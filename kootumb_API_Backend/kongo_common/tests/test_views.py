...
from kongo_common.tests.helpers import make_emoji_group, make_user, make_authentication_headers_for_user, \
    make_fake_post_text, make_proxy_blacklisted_domain

logger = logging.getLogger(__name__)


class TimeAPITests(kongoAPITestCase):
    """
    TimeAPITests
    """
...
class TestHealth(kongoAPITestCase):
    """
    Health API
    """
...
class TestEmojiGroups(kongoAPITestCase):
    """
    EmojiGroups API
    """
...
class ProxyDomainCheckAPITests(kongoAPITestCase):
    """
    ProxyDomainCheckAPI
    """
    ...
    def test_fails_on_blacklisted_domain(self):
        ...
        user = make_user()
        headers = make_authentication_headers_for_user(user)
        request_url = 'www.kootumb.io'
        url = self._get_url()
        make_proxy_blacklisted_domain(domain='kootumb.io')
