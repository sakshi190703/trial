"""
Django settings for kongo project.
...
from kongo_common.utils.environment import EnvironmentChecker
...
if not ENVIRONMENT:
    if 'test' in sys.argv:
        logger.info(
            'No ENVIRONMENT variable found but test detected. Setting ENVIRONMENT=TEST_VALUE')
...
if IS_BUILD and not IS_PRODUCTION:
    DEBUG = True
...
if IS_PRODUCTION:
    if not ALLOWED_HOSTS:
        raise NameError(
            'ALLOWED_HOSTS environment variable is required when running on a production environment')
    ALLOWED_HOSTS = [allowed_host.strip()
...
INSTALLED_APPS = [
    ...
    # 'django_nose',  # Commented out - not compatible with Python 3.12
    'corsheaders',  # Add CORS support for mobile app
    'ordered_model',
    'storages',
    'video_encoding',
    'imagekit',
    'django_media_fixtures',
    # 'cacheops',  # Temporarily disabled - requires Redis
    'django_rq',
    # 'scheduler',  # Temporarily commented out
    'django_extensions',
    'kongo_common',
    'kongo_auth',
    'kongo_posts',
...
MIDDLEWARE = [
    ...
    'kongo_common.middleware.TimezoneMiddleware'
]

ROOT_URLCONF = 'kongo.urls'

AUTH_USER_MODEL = 'kongo_auth.User'

# TEST_RUNNER = 'django_nose.NoseTestSuiteRunner'  # Commented out - not compatible with Python 3.12
TEST_RUNNER = 'django.test.runner.DiscoverRunner'  # Using default Django test runner
...
REDIS_DEFAULT_CACHE_LOCATION = '%(redis_location)s/%(db)d' % {
    'redis_location': REDIS_LOCATION, 'db': 0}
REDIS_RQ_DEFAULT_JOBS_CACHE_LOCATION = '%(redis_location)s/%(db)d' % {
    'redis_location': REDIS_LOCATION, 'db': 1}
REDIS_RQ_HIGH_JOBS_CACHE_LOCATION = '%(redis_location)s/%(db)d' % {
    'redis_location': REDIS_LOCATION, 'db': 2}
REDIS_RQ_LOW_JOBS_CACHE_LOCATION = '%(redis_location)s/%(db)d' % {
    'redis_location': REDIS_LOCATION, 'db': 3}
...
CACHEOPS_REDIS = '%(redis_location)s/%(db)d' % {
    'redis_location': REDIS_LOCATION, 'db': CACHEOPS_REDIS_DB}
...
WSGI_APPLICATION = 'kongo.wsgi.application'
...
if TESTING:
    ...
    RDS_HOSTNAME_READER = os.environ.get(
        'RDS_HOSTNAME_READER', RDS_HOSTNAME_WRITER)
    ...
    if IS_PRODUCTION:
        # Production: Use replication setup
        DATABASES = {
        ...
        # Development: Use simple single MySQL instance
        DATABASES = {
            'default': writer_db_config
        }
...
OS_TRANSLATION_CONFIG = {
    'default': {
        'STRATEGY': 'kongo_translation.strategies.amazon.AmazonTranslate',
        'TEXT_MAX_LENGTH': AWS_TRANSLATE_MAX_LENGTH,
        'DEFAULT_TRANSLATION_LANGUAGE_CODE': 'en'
    },
    'testing': {
        'STRATEGY': 'kongo_translation.strategies.tests.MockAmazonTranslate',
        'TEXT_MAX_LENGTH': 40,
        'DEFAULT_TRANSLATION_LANGUAGE_CODE': 'en'
    }
}
...
POST_COMMENT_MAX_HASHTAGS = int(
    os.environ.get('POST_COMMENT_MAX_HASHTAGS', '3'))
...
POST_LINK_MAX_DOMAIN_LENGTH = int(
    os.environ.get('POST_LINK_MAX_DOMAIN_LENGTH', '126'))
...
PROFILE_AVATAR_MAX_SIZE = int(os.environ.get(
    'PROFILE_AVATAR_MAX_SIZE', '10485760'))
PROFILE_COVER_MAX_SIZE = int(os.environ.get(
...
COMMUNITY_RULES_MAX_LENGTH = int(
    os.environ.get('COMMUNITY_RULES_MAX_LENGTH', '5000'))
COMMUNITY_CATEGORIES_MAX_AMOUNT = 3
COMMUNITY_CATEGORIES_MIN_AMOUNT = 1
COMMUNITY_AVATAR_MAX_SIZE = int(os.environ.get(
    'COMMUNITY_AVATAR_MAX_SIZE', '10485760'))
COMMUNITY_COVER_MAX_SIZE = int(os.environ.get(
...
FEATURE_IMPORTER_ENABLED = os.environ.get(
    'FEATURE_IMPORTER_ENABLED', 'True') == 'True'
MODERATION_REPORT_DESCRIPTION_MAX_LENGTH = 1000
MODERATED_OBJECT_DESCRIPTION_MAX_LENGTH = 1000
GLOBAL_HIDE_CONTENT_AFTER_REPORTS_AMOUNT = int(
    os.environ.get('GLOBAL_HIDE_CONTENT_AFTER_REPORTS_AMOUNT', '20'))
MODERATORS_COMMUNITY_NAME = os.environ.get('MODERATORS_COMMUNITY_NAME', 'mods')
PROXY_BLACKLIST_DOMAIN_MAX_LENGTH = 150
LINK_PREVIEW_TIMEOUT_IN_SECONDS = int(os.environ.get(
    'GLOBAL_HIDE_CONTENT_AFTER_REPORTS_AMOUNT', 8))
...
NEW_USER_SUGGESTED_COMMUNITIES = os.environ.get(
    'NEW_USER_SUGGESTED_COMMUNITIES', '1')
...
MIN_UNIQUE_TOP_POST_REACTIONS_COUNT = int(
    os.environ.get('MIN_UNIQUE_TOP_POST_REACTIONS_COUNT', '5'))
MIN_UNIQUE_TOP_POST_COMMENTS_COUNT = int(
    os.environ.get('MIN_UNIQUE_TOP_POST_COMMENTS_COUNT', '5'))
MIN_UNIQUE_TRENDING_POST_REACTIONS_COUNT = int(
    os.environ.get('MIN_UNIQUE_TRENDING_POST_REACTIONS_COUNT', '5'))
...
if IS_PRODUCTION:
    ...
    STATICFILES_STORAGE = 'kongo.storage_backends.S3StaticStorage'

    DEFAULT_FILE_STORAGE = 'kongo.storage_backends.S3PublicMediaStorage'

    PRIVATE_FILE_STORAGE = 'kongo.storage_backends.S3PrivateMediaStorage'
