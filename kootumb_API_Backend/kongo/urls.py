...
from kongo_auth.views.auth.views import Register, Login, UsernameCheck, EmailCheck, EmailVerify, \
    PasswordResetRequest, PasswordResetVerify, VerifyRegistrationToken
from kongo_auth.views.authenticated_user.views import AuthenticatedUser, AuthenticatedUserSettings, \
...
from kongo_auth.views.blocked_users.views import BlockedUsers, SearchBlockedUsers
from kongo_auth.views.followers.views import Followers, SearchFollowers
from kongo_auth.views.following.views import Followings, SearchFollowings
from kongo_auth.views.linked_users.views import LinkedUsers, SearchLinkedUsers
from kongo_auth.views.proxy.views import ProxyAuth
from kongo_auth.views.users.views import SearchUsers, GetUser, BlockUser, UnblockUser, \
    SubscribeToUserNewPostNotifications, GetUserPostsCount
from kongo_categories.views import Categories
from kongo_circles.views import Circles, CircleItem, CircleNameCheck
from kongo_common.views import Time, Health, EmojiGroups, ProxyDomainCheck, ChildSafetyPolicyView
from kongo_common.views_doc import APIDocumentationView
# from kongo_common.views import GuidelinesView
from kongo_communities.views.communities.views import Communities, TrendingCommunities, CommunityNameCheck, \
...
from kongo_communities.views.community.administrators.views import CommunityAdministratorItem, \
    CommunityAdministrators, SearchCommunityAdministrators
from kongo_communities.views.community.banned_users.views import BanUser, UnbanUser, CommunityBannedUsers, \
    SearchCommunityBannedUsers
from kongo_communities.views.community.members.views import CommunityMembers, JoinCommunity, \
    LeaveCommunity, InviteCommunityMember, SearchCommunityMembers, UninviteCommunityMember
from kongo_communities.views.community.moderators.views import CommunityModeratorItem, CommunityModerators, \
    SearchCommunityModerators
from kongo_communities.views.community.posts.views import CommunityPosts, ClosedCommunityPosts, \
    GetCommunityPostsCount
from kongo_communities.views.community.views import CommunityItem, CommunityAvatar, CommunityCover, \
    FavoriteCommunity, \
    SubscribeToCommunityNewPostNotifications, LegacyExcludeTopPostsCommunity
from kongo_connections.views import ConnectWithUser, Connections, DisconnectFromUser, UpdateConnection, \
    ConfirmConnection
from kongo_hashtags.views.hashtag.views import HashtagItem, HashtagPosts
from kongo_hashtags.views.hashtags.views import SearchHashtags
from kongo_invitations.views import UserInvite, UserInvites, SearchUserInvites, SendUserInviteEmail
from kongo_devices.views import Devices, DeviceItem
from kongo_follows.views import FollowUser, UnfollowUser, UpdateFollowUser, RequestToFollowUser, \
    ApproveUserFollowRequest, RejectUserFollowRequest, ReceivedFollowRequests, CancelRequestToFollowUser
from kongo_lists.views import Lists, ListItem, ListNameCheck
from kongo_moderation.views.checks import IsNotSuspendedCheck
from kongo_moderation.views.moderated_object.views import ModeratedObjectItem, ModeratedObjectLogs, \
    ApproveModeratedObject, RejectModeratedObject, VerifyModeratedObject, UnverifyModeratedObject, \
    ModeratedObjectReports
from kongo_moderation.views.moderated_objects.views import CommunityModeratedObjects, GlobalModeratedObjects
from kongo_moderation.views.moderation_categories.views import ModerationCategories
from kongo_moderation.views.report.views import ReportUser, ReportPost, ReportCommunity, \
    ReportPostComment, ReportHashtag
from kongo_moderation.views.user.views import UserModerationPenalties, UserPendingModeratedObjectsCommunities
from kongo_notifications.views import Notifications, NotificationItem, ReadNotifications, ReadNotification, \
    UnreadNotificationsCount
from kongo_posts.views.post.views import PostItem, PostOpen, PostClose, MutePost, UnmutePost, TranslatePost, \
    PostPreviewLinkData, SearchPostParticipants, GetPostParticipants, PublishPost, PostStatus
from kongo_posts.views.post_comment.post_comment_reaction.views import PostCommentReactionItem
from kongo_posts.views.post_comment.post_comment_reactions.views import PostCommentReactions, \
    PostCommentReactionsEmojiCount
from kongo_posts.views.post_comment.post_comment_replies.views import PostCommentReplies
from kongo_posts.views.post_comment.views import PostCommentItem, MutePostComment, UnmutePostComment, \
    TranslatePostComment
from kongo_posts.views.post_comments.views import PostComments, PostCommentsDisable, PostCommentsEnable
from kongo_posts.views.post_media.views import PostMedia
from kongo_posts.views.post_reaction.views import PostReactionItem
from kongo_posts.views.post_reactions.views import PostReactions, PostReactionsEmojiCount, PostReactionEmojiGroups
from kongo_posts.views.posts.views import Posts, TrendingPosts, TopPosts, TrendingPostsNew, \
    ...
    LinkIsPreviewable
from kongo_importer.views import ImportItem
...
auth_auth_patterns = [
    ...
    path('child-safety-policy/', ChildSafetyPolicyView.as_view(), name='child-safety-policy'),
    #path('child_safety/', ) ,# new api  created here ####################################################
    #################################################################################################### 
...
api_patterns = [
    ...
    re_path('time/', Time.as_view(), name='time'),
    re_path('emojis/groups/', EmojiGroups.as_view(), name='emoji-groups'),
    re_path('proxy-domain-check/', ProxyDomainCheck.as_view(),
...
urlpatterns = [
    re_path(r'^$', APIDocumentationView.as_view(), name='api_documentation'),
    path('api/', include(api_patterns)),
    re_path('admin/', admin.site.urls),
    path('django-rq/', include('django_rq.urls')),
    re_path('health/', Health.as_view(), name='health'),
]
...
if settings.DEBUG or settings.TESTING:
    urlpatterns.append(re_path('proxy/(?P<url>.*)', proxy_view, name='proxy'), )
